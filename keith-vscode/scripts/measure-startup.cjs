...
// page cache and (for AppCDS) the archive dump, and is reported separately.
//
...
const STATIC = [`-XX:SharedArchiveFile=${archive}`, '-Xshare:auto']
const CONFIGS = {
    baseline: [],
    cds: CDS,
...
async function measure(java, flags, compile) {
    ...
    const server = spawn(java, ['-Djava.awt.headless=true', ...flags, '-cp', jar, MAIN], { cwd: workspace })
    let stderr = ''
    ...
    const waitFor = (method, accept = () => true) => new Promise((resolve, reject) => {
        const waiter = { method, accept, resolve: (r) => { clearTimeout(timer); waiters.delete(waiter); resolve(r) } }
        const timer = setTimeout(() => { waiters.delete(waiter); reject(new Error(`Timeout waiting for ${method}\n${stderr}`)) }, 120000)
        waiters.add(waiter)
    })
    connection.listen()
    const result = {}
    try {
        ...
        await connection.sendNotification('textDocument/didOpen', { textDocument: { uri, languageId: 'sctx', version: 1, text: fs.readFileSync(model, 'utf8') } })
        ...
        if (compile) {
            ...
            await connection.sendNotification('textDocument/didOpen', { textDocument: { uri: brokenUri, languageId: 'sctx', version: 1, text: fs.readFileSync(broken, 'utf8') } })
...
function summarise(runs) {
    ...
    for (const key of keys) {
        ...
        if (values.length) summary[key] = values.reduce((a, b) => a + b, 0) / values.length
    }
    return summary
}

async function main() {
    const java = option('--java', process.env.SCCHARTS_JAVA ?? 'java')
    ...
    for (const name of names) {
        ...
        if (config.static) {
            ...
            const dump = spawnSync(java, ['-Xshare:dump', `-XX:SharedClassListFile=${classlist}`, `-XX:SharedArchiveFile=${archive}`, '-Djava.awt.headless=true', '-cp', jar], { encoding: 'utf8' })
    ...
    for (const row of rows) {
        console.log(`${row.config} | ${fmt(row.first.initialize)} | ${fmt(row.warm.initialize)} | ${fmt(row.warm.systems)} | ${fmt(row.warm.compile)} | ${fmt(row.warm.rss / 1e6)} | ${row.archive ? (row.archive / 1e6).toFixed(1) : '-'} | ${fmt(row.dumpMs)}`)
    }
