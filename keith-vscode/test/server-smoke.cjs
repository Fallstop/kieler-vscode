...
async function main() {
    ...
    const server = spawn('java', ['-Djava.awt.headless=true', '-cp', `${serverDir}/jetty10/*${path.delimiter}${serverDir}/kieler-language-server.jar`, 'de.cau.cs.kieler.language.server.LanguageServer'], { cwd: extension })
